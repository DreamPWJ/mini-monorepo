...
export class WeiXin {
  ...
  static uploadActionSheet = () => new Promise((resolve, reject) => {
    Taro.showActionSheet({
      itemList: ['拍照', '从相册选择'],
      success: (res) => {
        WeiXin.chooseImage(res.tapIndex).then((result) => {
          resolve(result)
        })
      },
      fail: (error) => {
  ...
  static chooseImage = (type?: number | 'default', count = 6, isUpload = true) => new Promise<any>((resolve, reject) => {
    ...
    Taro.chooseImage({
      ...
      success: (res) => {
        if (isUpload) {
          // 上传文件
          WeiXin.uploadFile(res).then(result => {
            resolve(result)
          })
        }
        resolve(res)
        ...
        success: (res) => {
          ...
          if (code === 200) {
            ...
            if (tempFilePaths.length === index + 1) {
              setTimeout(() => {
                resolve(imageUrls)
                //WeiXin.showToast("图片上传成功","success");
              }, 600)
            }
          } else {
  ...
  /* static webSocket = () => {
     ...
     const connectSocket = () => {
       Taro.connectSocket({
         url: `${process.env.WEB_SOCKET}/websocket?userId=${userId}`,
         success: () => {
           // 监听 WebSocket 打开
           Taro.onSocketOpen(() => {
             //	发送 WebSocket 消息
             ...
             Taro.onSocketMessage((res) => {
               try {
                 ...
                 // if (result.key != 'open') EventBus.publish(`webSocket:${result.key}`, result.data);
                 console.log(result)
               } catch (e) {
  ...
  static checkForUpdate = () => {
    ...
    updateManager.onUpdateReady(() => {
      WeiXin.confirm('更新提示', '新版本已经准备好，是否更新重启应用？',
        '立即更新', '稍后更新').then(res => {
        if (res.confirm) {
  ...
  static makePhoneCall = (phoneNumber, userName, isShowConfirm = true) => {
    ...
    if (isShowConfirm) {
      WeiXin.confirm('打电话', `是否拨打 ${userName} 的电话 ${phoneNumber}`, '拨打').then(res => {
        if (res.confirm) {
          Taro.makePhoneCall({
